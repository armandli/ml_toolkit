...
class GridGame : public ML::GameModelIF {
...
public:
  ...
  virtual void init() final {
    ...
    while (not_found){
      int a = dist(ML::get_default_random_engine()), b = dist(ML::get_default_random_engine());
      if (board1[a][b] == 0){
        board1[a][b] ^= PMASK;
        not_found = false;
      }
  ...
  virtual ML::Mtx to_input() final {
    ML::Mtx ret(1, GridSize * GridSize * 4);
...
void play(Model& model){
  ...
  for (int i = 0; i < 40 && reward != 10. && reward != -10.; ++i){
    ML::Mtx X = game.to_input();
    ML::Mtx Y = model.predict(X);
    vector<ML::DimV> mcoeffs = Y.max_coeff(ML::MRow);
    int action = mcoeffs[0].idx;
...
int main(){
  ...
  ML::RFL<decltype(model)> rflearner(game, model);
