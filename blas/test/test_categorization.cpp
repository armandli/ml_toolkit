...
using namespace std;
using ML::Mtx;
using Model = ML::FFN<ML::CrossEntropy, ML::ReluFun, ML::L2Reg, ML::AdamUpdate, false>;
...
void create_input(const vector<double> v, Mtx& trainX, Mtx& trainY, Mtx& testX, Mtx& testY){
  ...
  trainX = Mtx(total_rows - test_rows.size(), 3);
  trainY = Mtx(total_rows - test_rows.size(), 3);
  testX  = Mtx(test_rows.size(), 3);
  testY  = Mtx(test_rows.size(), 3);
